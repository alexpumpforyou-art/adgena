...
const MARKETPLACE_SAFE_RU = `ВАЖНО ДЛЯ МАРКЕТПЛЕЙСА: товар должен быть главным объектом и занимать 55-70% кадра. Используй максимум 3-5 коротких преимуществ, не больше 2-4 слов каждое. Не добавляй мелкий нечитаемый текст, случайные цифры, артикулы, SKU, штрихкоды, выдуманные сертификаты или характеристики. Артикул/код товара можно писать только если пользователь явно указал его в тексте. Оставь безопасные поля 5-7% от краёв, чтобы текст не обрезался. Дизайн должен выглядеть как готовая карточка для Wildberries/Ozon: чисто, контрастно, продающе.`;
const MARKETPLACE_SAFE_EN = `MARKETPLACE REQUIREMENTS: the product must be the main object and occupy 55-70% of the frame. Use up to 3-5 short benefits, 2-4 words each. Do not add tiny unreadable text, random numbers, article numbers, SKUs, barcodes, fake certifications, or invented specs. Product article/SKU may be written only if the user explicitly provided it in text. Keep a safe margin of 5-7% from all edges so text is not cropped. The design must look like a finished marketplace card: clean, high-contrast, conversion-focused.`;
